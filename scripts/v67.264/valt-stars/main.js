...
const state = { aimbot: false, autododge: false, killaura: false, followMates: false, followEnemies: false };
...
const OFFSETS_X86_FUNC = {
    LogicBattleModeClient_update: 0xBC1FE0,
    BattleMode_getInstance: 0x91BFE0,
    LogicGameObjectClient_getX: 0xAABB1C,
    LogicGameObjectClient_getY: 0xAABB24,
    LogicBattleModeClient_getOwnCharacter: 0xBDB4D8,
    BattleScreen_activateSkill: 0x84D64C,
    BattleScreen__updateMovement: 0x846248,
    BattleScreen__updateAutoshoot: 0x8487A0,
    LogicBattleModeClient_setClientPredictionMoveTo: 0xBDB674,
    ClientInput_constructor_int: 0xB13498,
    ClientInputManager_addInput: 0x7D50C4,
    GameScreen__getLogicBattle: 0x857ACC,
    LogicTileMap_getTile: 0x993070,
    StringCtor: 0xD83910,
    StringTable_getMovieClip: 0xBA1CA0,
    GameButtonCtor: 0x5D7AB8,
    CustomButton_SetMovieClip: 0x5D96FC,
    MovieClip_gotoAndStopFrameIndex: 0xC5278C,
    DisplayObject_setXY: 0xC5D133,
    TextField_setText_ui: 0x5D38E7,
    MovieClip_getTextFieldByName: 0xC52348,
    MovieClip_setChildVisible: 0xC51C50,
    TextField_setText: 0xC5E580,
    ResourceListener_addFile: 0xCD5080,
    Stage_addChild: 0xC59A4C,
    Sprite_addChild: 0xC629C4,
    ButtonCallback: 0xC52C48,
    BattleMode_enter: 0x919764,
    VTABLE_PROJECTILE_DATA: 0x111F0B0,
    LogicProjectileData__isBeam: 0xACE670,
    LogicCharacterClient__getLinkedCarryable: 0xAFF8D8,
    LogicPlayerMapUtil__tileDataToTileCode: 0xB66010,
};
...
function resolveOffsets() {
    ...
    if (arch === 'x64' || arch === 'ia32') {
        Object.assign(o, OFFSETS_X86_FUNC);
        o.Killaura_nativeFire = 0x80621F;
    } else {
        ...
        for (let i = 0; i < count; i++) {
            try {
                ...
                if (isProjectile) {
                    ...
                    if (obj.equals(own)) continue;
                    if (obj.add(offsets.GameObj_deadFlag).readU32() !== 0) continue;

                    // Allie (meme equipe) : collecte pour le mode Follow Mates
                    if (!isEnemy) {
                        allies.push({
                            x: functions.LogicGameObjectClient_getX(obj) | 0,
                            y: functions.LogicGameObjectClient_getY(obj) | 0
...
// =================== FOLLOW (Mates / Enemies) ===================
let _followDir = null;
const FOLLOW_MIN_DIST = 260;   // ne pas coller a la cible
const FOLLOW_MAX_DIST = 9000;  // ignorer les cibles trop loin

function updateFollow() {
    if (!state.followMates && !state.followEnemies) { _followDir = null; return; }
    if (scanData.lastUpdate === 0) { _followDir = null; return; }

    const myX = scanData.myX, myY = scanData.myY;
    const myRadius = scanData.myRadius || 60;

    const list = state.followMates ? (scanData.allies || []) : (scanData.enemies || []);

    // Cible la plus proche dans la plage de distance
    let best = null, bestD = Infinity;
    for (const t of list) {
        const dx = t.x - myX, dy = t.y - myY;
        const d = Math.sqrt(dx * dx + dy * dy);
        if (d < FOLLOW_MIN_DIST || d > FOLLOW_MAX_DIST) continue;
        if (d < bestD) { bestD = d; best = t; }
    }
    if (!best) { _followDir = null; return; }

    const dx = best.x - myX, dy = best.y - myY;
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist < 1) { _followDir = null; return; }
    const dirX = dx / dist, dirY = dy / dist;

    // Direction directe si marchable (isDirectionWalkable gere murs ET eau via blocksMovement)
    if (isDirectionWalkable(myX, myY, dirX, dirY, myRadius)) {
        _followDir = { x: dirX, y: dirY };
        return;
    }
    // Sinon : meilleure direction marchable orientee vers la cible (contournement)
    let bestDir = null, bestScore = -Infinity;
    for (let i = 0; i < CACHED_DIRECTIONS.length; i++) {
        const dd = CACHED_DIRECTIONS[i];
        if (!isDirectionWalkable(myX, myY, dd.x, dd.y, myRadius)) continue;
        const dot = dd.x * dirX + dd.y * dirY;
        if (dot > bestScore) { bestScore = dot; bestDir = dd; }
    }
    _followDir = (bestDir && bestScore > -0.5) ? bestDir : null;
}
...
function setupAutododge(base) {
    Interceptor.attach(base.add(offsets.BattleScreen__updateMovement), {
        onEnter: function (args) {
            let dir = null;
            // PRIORITE AU DODGE : si une esquive est active, elle prime sur le follow
            if (state.autododge && _dodgeDir) {
                dir = _dodgeDir;
            } else if ((state.followMates || state.followEnemies) && _followDir) {
                dir = _followDir;
            }
            if (!dir) return;
...
// Positions de l'UI (Menu en haut a gauche ; fenetre = colonne en dessous)
const POS = {
    menu:     { x: 60,  y: 90  },   // bouton Menu (cyan, toujours visible)
    killaura: { x: 60,  y: 140 },
    aimbot:   { x: 60,  y: 185 },
    dodge:    { x: 60,  y: 230 },
    follow:   { x: 60,  y: 275 },
    mates:    { x: 152, y: 275 },   // sous-menu Follow, a droite
    enemies:  { x: 244, y: 275 },
    discord:  { x: 60,  y: 360 },   // lien YaduStars en bas de la fenetre
};
const HIDDEN = -9999;

let menuBtn = null,     menuTextField = null;
let dodgeBtn = null,    dodgeTextField = null;
let aimbotBtn = null,   aimbotTextField = null;
let killauraBtn = null, killauraTextField = null;
let followBtn = null,   followTextField = null;
let matesBtn = null,    matesTextField = null;
let enemiesBtn = null,  enemiesTextField = null;
let discordBtn = null,  discordTextField = null;

let capturedParent = null;
let capturedAddChildAddr = null;
let buttonsCreated = false;
let panelOpen = false;       // fenetre ouverte ?
let followSubOpen = false;   // sous-menu Follow deroule ?
let _lastBtnTry = 0;
let _creatingButtons = false;
...
function moveBtn(btn, x, y) {
    if (!btn || btn.isNull()) return;
    try { getFunctions().DisplayObject_setXY(btn, x, y); } catch (_) {}
}

// Ouvre/ferme la fenetre (montre/cache les boutons du mod). Le bouton Menu reste cyan.
function setPanelVisible(open) {
    panelOpen = open;
    if (open) {
        moveBtn(killauraBtn, POS.killaura.x, POS.killaura.y);
        moveBtn(aimbotBtn,   POS.aimbot.x,   POS.aimbot.y);
        moveBtn(dodgeBtn,    POS.dodge.x,    POS.dodge.y);
        moveBtn(followBtn,   POS.follow.x,   POS.follow.y);
        moveBtn(discordBtn,  POS.discord.x,  POS.discord.y);
        if (followSubOpen) setFollowSubVisible(true);
    } else {
        moveBtn(killauraBtn, HIDDEN, HIDDEN);
        moveBtn(aimbotBtn,   HIDDEN, HIDDEN);
        moveBtn(dodgeBtn,    HIDDEN, HIDDEN);
        moveBtn(followBtn,   HIDDEN, HIDDEN);
        moveBtn(discordBtn,  HIDDEN, HIDDEN);
        moveBtn(matesBtn,    HIDDEN, HIDDEN);
        moveBtn(enemiesBtn,  HIDDEN, HIDDEN);
    }
    if (menuTextField) setButtonTextColor(menuTextField, RGBA.cyan);
}

// Deroule/replie le sous-menu Follow. Jaune = sous-menu ouvert.
function setFollowSubVisible(open) {
    followSubOpen = open;
    if (open) {
        moveBtn(matesBtn,   POS.mates.x,   POS.mates.y);
        moveBtn(enemiesBtn, POS.enemies.x, POS.enemies.y);
        if (followTextField) setButtonTextColor(followTextField, RGBA.yellow);
    } else {
        moveBtn(matesBtn,   HIDDEN, HIDDEN);
        moveBtn(enemiesBtn, HIDDEN, HIDDEN);
        if (followTextField) setButtonTextColor(followTextField, RGBA.white);
    }
}
...
function createAllButtons() {
    ...
    try {
        ...
        // Menu (cyan, toujours visible)
        try { const r = createButton("Menu", POS.menu.x, POS.menu.y, capturedParent, addChild, 0);
            if (r) { menuBtn = r.btn; menuTextField = r.textField; setButtonTextColor(menuTextField, RGBA.cyan); } } catch (_) { menuBtn = null; }

        // Boutons de la fenetre (caches au depart)
        try { const r = createButton("Killaura", HIDDEN, HIDDEN, capturedParent, addChild, 1);
            if (r) { killauraBtn = r.btn; killauraTextField = r.textField; setButtonTextColor(killauraTextField, state.killaura ? RGBA.green : RGBA.red); } } catch (_) { killauraBtn = null; }

        try { const r = createButton("Aimbot", HIDDEN, HIDDEN, capturedParent, addChild, 1);
            if (r) { aimbotBtn = r.btn; aimbotTextField = r.textField; setButtonTextColor(aimbotTextField, state.aimbot ? RGBA.green : RGBA.red); } } catch (_) { aimbotBtn = null; }

        try { const r = createButton("Autododge", HIDDEN, HIDDEN, capturedParent, addChild, 1);
            if (r) { dodgeBtn = r.btn; dodgeTextField = r.textField; setButtonTextColor(dodgeTextField, state.autododge ? RGBA.green : RGBA.red); } } catch (_) { dodgeBtn = null; }

        try { const r = createButton("Follow", HIDDEN, HIDDEN, capturedParent, addChild, 0);
            if (r) { followBtn = r.btn; followTextField = r.textField; setButtonTextColor(followTextField, followSubOpen ? RGBA.yellow : RGBA.white); } } catch (_) { followBtn = null; }

        try { const r = createButton("Mates", HIDDEN, HIDDEN, capturedParent, addChild, 1);
            if (r) { matesBtn = r.btn; matesTextField = r.textField; setButtonTextColor(matesTextField, state.followMates ? RGBA.green : RGBA.red); } } catch (_) { matesBtn = null; }

        try { const r = createButton("Enemies", HIDDEN, HIDDEN, capturedParent, addChild, 1);
            if (r) { enemiesBtn = r.btn; enemiesTextField = r.textField; setButtonTextColor(enemiesTextField, state.followEnemies ? RGBA.green : RGBA.red); } } catch (_) { enemiesBtn = null; }

        // Lien discord (label, en bas de la fenetre)
        try { const r = createButton("YaduStars", HIDDEN, HIDDEN, capturedParent, addChild, 0);
            if (r) { discordBtn = r.btn; discordTextField = r.textField; setButtonTextColor(discordTextField, RGBA.cyan); } } catch (_) { discordBtn = null; }

        // Cree seulement si le bouton Menu a bien ete cree (asset UI pret), sinon on reessaie
        buttonsCreated = !!(menuBtn && !menuBtn.isNull());
        if (!buttonsCreated) return;
        // restaurer l'etat visuel si la fenetre etait ouverte
...
function setupUI(base) {
    // Re-essaie tant que les boutons ne sont pas crees (asset UI pas pret sur install propre,
    // ecran de login transitoire...). _creatingButtons evite la recursion (createButton -> addChild).
    Interceptor.attach(base.add(offsets.Sprite_addChild), {
        onEnter(args) {
            if (!buttonsCreated && !_creatingButtons) {
                const now = Date.now();
                if (now - _lastBtnTry >= 1000) {
                    _lastBtnTry = now;
                    _creatingButtons = true;
                    capturedParent = args[0];
                    setCapturedAddChildAddr(base.add(offsets.Sprite_addChild));
                    try { createAllButtons(); } catch (_) {}
                    _creatingButtons = false;
                }
            }
        },
    });
    ...
    Interceptor.attach(base.add(offsets.ButtonCallback), {
        onEnter(args) {
            try {
                ...
                const a = clicked.toInt32();

                // Menu : ouvre/ferme la fenetre
                if (menuBtn && !menuBtn.isNull() && a === menuBtn.toInt32()) {
                    setPanelVisible(!panelOpen); return;
                }
                if (!panelOpen) return;

                // Follow : deroule/replie Mates + Enemies
                if (followBtn && !followBtn.isNull() && a === followBtn.toInt32()) {
                    setFollowSubVisible(!followSubOpen); return;
                }

                if (killauraBtn && !killauraBtn.isNull() && a === killauraBtn.toInt32()) {
                    state.killaura = !state.killaura;
                    setButtonTextColor(killauraTextField, state.killaura ? RGBA.green : RGBA.red);
                }
                if (aimbotBtn && !aimbotBtn.isNull() && a === aimbotBtn.toInt32()) {
                    state.aimbot = !state.aimbot;
                    setButtonTextColor(aimbotTextField, state.aimbot ? RGBA.green : RGBA.red);
                }
                if (dodgeBtn && !dodgeBtn.isNull() && a === dodgeBtn.toInt32()) {
                    state.autododge = !state.autododge;
                    setButtonTextColor(dodgeTextField, state.autododge ? RGBA.green : RGBA.red);
                }

                if (!followSubOpen) return;
                if (matesBtn && !matesBtn.isNull() && a === matesBtn.toInt32()) {
                    state.followMates = !state.followMates;
                    state.followEnemies = false;
                    if (!state.followMates) _followDir = null;
                    setButtonTextColor(matesTextField,   state.followMates ? RGBA.green : RGBA.red);
                    setButtonTextColor(enemiesTextField, RGBA.red);
                }
                if (enemiesBtn && !enemiesBtn.isNull() && a === enemiesBtn.toInt32()) {
                    state.followEnemies = !state.followEnemies;
                    state.followMates = false;
                    if (!state.followEnemies) _followDir = null;
                    setButtonTextColor(enemiesTextField, state.followEnemies ? RGBA.green : RGBA.red);
                    setButtonTextColor(matesTextField,   RGBA.red);
                }
            } catch (e) {}
        },
    });
    ...
    Interceptor.attach(base.add(offsets.BattleMode_enter), {
        onEnter() {
            capturedParent = null; capturedAddChildAddr = null;
            menuBtn = null; killauraBtn = null; aimbotBtn = null; dodgeBtn = null;
            followBtn = null; matesBtn = null; enemiesBtn = null; discordBtn = null;
            menuTextField = null; killauraTextField = null; aimbotTextField = null; dodgeTextField = null;
            followTextField = null; matesTextField = null; enemiesTextField = null; discordTextField = null;
            buttonsCreated = false; panelOpen = false; followSubOpen = false;
        },
    });
}
    ...
    return libc().then((c) => {
      ...
      Interceptor.attach(base.add(offsets.LogicBattleModeClient_update), {
        onEnter: function (args) {
          ...
          if (!(state.aimbot || state.killaura || state.autododge || state.followMates || state.followEnemies)) return;

          updateScanner(bm);
          if (state.aimbot || state.killaura) updateAimbot();
          if (state.killaura) updateKillaura();
          if (state.autododge) updateAutododge();
          if (state.followMates || state.followEnemies) updateFollow();
        }
      });
